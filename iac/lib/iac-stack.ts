...
export class IacStack extends cdk.Stack {
  constructor(scope: Construct, id: string, props?: cdk.StackProps) {
    ...
    const projectName = process.env.PROJECT_NAME || 'CleanMssTsTemplate'

    const s3Bucket = new s3.Bucket(this, 'CleanMssTsTemplateBucket' + stage, {
      versioned: true,
    ...
    const oac = new cloudfront.CfnOriginAccessControl(this, 'AOC', {
      originAccessControlConfig: {
        name: 'Typescript Template Bucket OAC ' + stage,
    ...
    if (stage === 'prod' || stage === 'homolog' || stage === 'dev') {
      viewerCertificate = cloudfront.ViewerCertificate.fromAcmCertificate(
        Certificate.fromCertificateArn(
          this,
          'CleanMssTsTemplateCertificate-' + stage,
          acmCertificateArn
        ),
        {
    ...
    const cloudFrontWebDistribution = new cloudfront.CloudFrontWebDistribution(
      ...
      'CDN',
      {
        comment: 'Typescript Template Distribution ' + stage,
        originConfigs: [
          {
            s3OriginSource: {
    ...
    if (stage === 'prod' || stage === 'homolog' || stage === 'dev') {
      const zone = route53.HostedZone.fromHostedZoneAttributes(
        this,
        'CleanMssTsTemplateHostedZone-' + stage,
        {
      ...
      new route53.ARecord(this, 'CleanMssTsTemplateAliasRecord-' + stage, {
        zone: zone,
    ...
    new cdk.CfnOutput(this, 'CleanMssTsTemplateBucketName-' + stage, {
      value: s3Bucket.bucketName
    })

    new cdk.CfnOutput(this, 'CleanMssTsTemplateDistributionId-' + stage, {
      value: cloudFrontWebDistribution.distributionId
    })

    new cdk.CfnOutput(
      this,
      'CleanMssTsTemplateDistributionDomainName-' + stage,
      {
